# Implementação do Módulo `data_to_entity_engine.py`

Este documento contém o código completo para o novo módulo `data_to_entity_engine.py`, que implementa a transformação de dados para o formato `entity_engine` conforme solicitado.
...
Para usar este módulo, importe a função `transform_to_entity_engine` e passe as listas `output_data` e `output_hierarchical` como parâmetros:

```python
from data_to_entity_engine import transform_to_entity_engine
import json

# Exemplo de uso (não carrega de arquivos, recebe diretamente as listas)
output_data = [...]  # Lista de dados
output_hierarchical = {...}  # Dicionário com estrutura hierárquica

# Transformar para o formato entity_engine
result = transform_to_entity_engine(output_data, output_hierarchical)

# Opcional: Salvar o resultado em um arquivo
with open('entity_engine_result.json', 'w', encoding='utf-8') as f:
    json.dump(result, f, indent=4, ensure_ascii=False)
```

## Principais Diferenças em Relação ao Módulo Original

1. A função principal foi renomeada para `transform_to_entity_engine`
2. Os tipos numéricos são mapeados para "numeric" em vez de "number"
3. A função recebe diretamente as listas de dados, sem carregar de arquivos
4. Removido o bloco de execução standalone (`if __name__ == '__main__':`)
